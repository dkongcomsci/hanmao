# ADR 0005 — วงมี host เจ้าของเดียว + เข้าวงผ่าน RPC เท่านั้น

- สถานะ: Accepted
- วันที่: 2026-07-31

## บริบท
โครง RLS รุ่นแรกให้สิทธิ์ทุกอย่างกับ "คนที่เป็นสมาชิกวง" (`group_participants`) เท่ากันหมด
ทำให้เกิดสองช่องโหว่ที่เอาข้อมูลของคนอื่นหายได้จริง:

1. **สมาชิกคนไหนก็ลบวงทั้งวงได้** — ปุ่ม "ปิดวง" ในหน้าสรุปกดโดยใครก็ได้ แล้ว cascade
   ลบสมาชิก/บิล/เมนูของทุกคนถาวร
2. **ใครรู้ `groups.id` (uuid) ก็ insert ตัวเองเข้าวงได้** โดยไม่ต้องมีโค้ดเชิญ
...
2. ทำระบบ role/permission ในตารางแยก (owner/member/…) — ยืดหยุ่นกว่า แต่เกินความจำเป็น
   ของ MVP ที่ต้องแยกแค่ "คนสร้างวง" กับ "คนอื่น"
...
- เพิ่ม `groups.created_by` (default `auth.uid()`, FK ไป `auth.users`) + backfill ของวงเดิม
  โดยใช้ participant คนแรกที่เข้าวงเป็น host
...
- policy `gp_insert` → insert `group_participants` ตรงได้แค่ผู้สร้างวง (ตอน `createGroup`);
...
host = "ปิดวง (ลบของทุกคน)", ไม่ใช่ host = "ออกจากวง"
...
- คนที่ไม่ใช่ host ไม่มีทางลบวงได้อีก ถ้า host หายไปจากวง วงจะค้างอยู่ (ยอมรับใน MVP;
  มีข้อยกเว้นในทริกเกอร์สำหรับ "วงกำพร้า" ที่ `created_by` เป็น null ให้ claim ได้)

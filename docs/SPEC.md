...
> เวอร์ชัน: dual-mode (local + กลุ่มหลายคน real-time ผ่าน Supabase) · UI ภาษาไทย · ธีมสว่าง/มืดสลับได้ (default มืด) · iOS / Android / Web จาก codebase เดียว
...
1. ในหนึ่งกลุ่มมี **หลายบิล** (เช่น ค่าอาหาร บิลนึง ค่าเหล้าอีกบิลนึง) และ **คนจ่ายแต่ละบิลอาจเป็นคนละคน**
2. คนในกลุ่ม **มาไม่พร้อมกัน / กลับไม่พร้อมกัน** → บางคนไม่ควรร่วมจ่ายบางบิล
...
- **group mode** (หลายคน real-time) — เข้าร่วม "กลุ่ม" ผ่าน invite link/QR แล้ว state อยู่บน **Supabase** (Postgres + Realtime) ทุกคนแก้พร้อมกันแล้ว sync
...
`hanmao:state:v1` (state ของ local mode) · `hanmao:session:v1` (อยู่กลุ่มไหน + เราเป็น member ไหนในกลุ่ม) · `hanmao:me:v1` ("ฉันคือใคร" ของ local mode) · `hanmao:theme:v1` (ธีมสว่าง/มืด — device preference, ไม่ผูกโหมด ดู §5.1)
...
> meta ของกลุ่มปัจจุบัน (`Group = { id, name, inviteCode }`), `mode`, และ `myMemberId` ("ฉันคือใคร") ไม่ได้อยู่ใน `AppState` แต่เก็บแยกใน store (ดู §6)
...
| `splitMode` | `'equal' \| 'itemized' \| 'time'` | วิธีหาร (ดู §4) — โหมด `time` ยังอยู่ในโมเดล/สูตร แต่ **ปิดจาก UI ชั่วคราว** (ดู §4) |
...
> **โหมด `time` ปิดจาก UI ชั่วคราว**: `bill/[id].tsx` ให้เลือกได้แค่ `equal`/`itemized` (`MODES = ['equal','itemized']`) และปุ่ม "มาถึง"/"กลับ" ใน `members.tsx` ถูกปิด. สูตรโหมด `time` ทั้งหมด (รวม star / `stamp` / `amountsDrift` ใน §4.5–4.6) **ยังอยู่ครบใน `split.ts`** และบิลเก่าที่เป็น `time` ยังคำนวณได้ตามเดิม — ส่วน §4.5–4.6 ด้านล่างยังคงเป็นสเปกที่ถูกต้องของ logic
...
ทุกหน้าที่คิดเงินทำแบบนี้แล้ว: `bills.tsx`, `bill/[id].tsx`, `summary.tsx`, `me.tsx` (`index.tsx` ไม่คิดเงินแล้ว — เอา hero ยอดรวมออก)
...
- เหตุผลที่ต้องแยกโหมด: greedy จับคู่จากลำดับยอดที่ลอยตามเวลา ⇒ **ตัวคู่โอนเปลี่ยนเอง** ทั้งที่ผู้ใช้ไม่ได้แก้อะไร
  ⇒ ติ๊ก "โอนแล้ว" หลุด ⇒ กล่อง 🎉 กับปุ่มปิดกลุ่มหายเอง. ต้นทุนของ star = รายการโอนมากกว่า greedy
  ราว +0.1% (บางกลุ่มที่มีเจ้าหนี้หลายคนเพิ่ม 1 รายการ) — ดู [adr/0006-stable-settle-topology.md](adr/0006-stable-settle-topology.md)
...
(สำคัญในโหมดกลุ่ม: Postgres ไม่การันตีลำดับแถว ⇒ สองเครื่องอาจได้ลำดับต่างกัน ⇒ ถ้าผลต่างกัน คนหนึ่งจะเห็น "โอนแล้ว" อีกคนเห็น "ยังไม่โอน")
...
- **`transferKey(t)` มี 2 รูปแบบ** ตามว่ารายการนั้นมี `stamp` ไหม (มาจาก `settleUp`) — **ห้ามประกอบ key เองเด็ดขาด**:
  ...
  - **ตาข่ายกันเงินหาย**: ตอนคนสุดท้ายติ๊ก "กลับแล้ว" ยอดหยุดลอย → key ย้ายจากแบบ stamp กลับไปแบบผูกยอด
    ⇒ **ติ๊กชุดเดิมเป็นโมฆะทั้งชุด ต้องติ๊กใหม่บนยอดปิดจริงก่อนปิดกลุ่ม** — เป็นราคาที่ยอมจ่ายเพื่อความหลวมของ key แบบ stamp
...
- **`nothingOwed(transfers, settlements)` → boolean** — `true` เมื่อ **ไม่มีรายการโอนเลย** (หารลงตัวพอดี ไม่มีใครต้องโอน) **หรือ** ทุกรายการถูกติ๊กว่าโอนแล้ว
  หน้าสรุปใช้ปลดล็อกกล่อง 🎉 + ปุ่มเคลียร์/ปิดกลุ่ม (คู่กับเงื่อนไข "มีข้อมูลให้สรุปจริง")
...
Navigation = **bottom tabs** 5 แท็บ (หน้าแรก/สมาชิก/บิล/สรุป/ฉัน — ตั้งใน `app/_layout.tsx`) + หน้าที่ซ่อนจากแท็บ: รายละเอียดบิล (`bill/[id]`), จัดการกลุ่ม (`group`), เข้าร่วมกลุ่ม (`join/[code]`)
...
- ธีมเป็น **device preference**: ไม่อยู่ใน `AppState`, ไม่ sync ขึ้น Supabase, ไม่ผูกกับ local/group mode
  (คนในกลุ่มเดียวกันเลือกธีมคนละแบบได้ — persist ทุกโหมด) — ดู [adr/0007-runtime-theme-switch.md](adr/0007-runtime-theme-switch.md)

### 🏠 หน้าแรก (`index.tsx`)
- **onboarding "เริ่มใช้งานใน 3 ขั้น"** (เพิ่มสมาชิก → สร้างบิล → ดูสรุป) — แสดง **เสมอ** ไม่ใช่เฉพาะตอนว่าง (ไม่มี hero ยอดรวมแล้ว)
- **ปุ่มไอคอนสี่เหลี่ยมจัตุรัส 3 ปุ่มเรียงแถว**:
  - 🧍 **หารคนเดียว** → ไปหน้าสมาชิก (`/members`)
  - 👥 **หารร่วมกันหลายคน** → ไปหน้ากลุ่ม (`/group`)
  - 🧾 **เริ่มทำรายการใหม่** → เรียก `reset()` ผ่าน `confirmAction` (ลบสมาชิก+บิลทั้งหมด เริ่มใหม่เหมือนเข้าครั้งแรก, **ลบถาวร**); ถ้าไม่มีข้อมูลอยู่แล้ว = no-op
- **แถบกลุ่ม (`groupBar`)** — แสดงเมื่ออยู่ในกลุ่มแล้ว (`remoteEnabled && mode === 'group' && group`): ชื่อกลุ่ม + แตะเพื่อไปหน้ากลุ่ม (QR/ลิงก์เชิญ)
...
- แต่ละคน: แก้ `consumes` ได้, แก้พร้อมเพย์แบบ inline (**บันทึกเมื่อกดปุ่ม "บันทึก"** ที่โผล่เฉพาะเมื่อมีการแก้ไข dirty และกดได้เมื่อค่าถูกต้อง — ไม่ auto-save ตอน blur แล้ว; ถ้าเพื่อนในกลุ่มแก้มาทาง realtime ขณะเราไม่ได้พิมพ์ค้าง ค่าจะ sync ตาม), ปุ่ม **ลบ** (`confirmRemove` ยืนยันก่อน)
- **ปุ่ม "มาถึง"/"กลับ" (toggle เวลา `arrivedAt`/`leftAt`) ถูกปิดไว้ชั่วคราว** — ใช้กับโหมดหารตามเวลาที่ปิดอยู่ (ดู §4); store ยังมี `toggleArrived`/`toggleLeft` ครบ แค่ UI ไม่แสดงปุ่ม
...
- `bill/[id].tsx` (หน้าซับซ้อนสุด) — **แก้แบบ draft**: ทุกช่องหน่วงลง draft state ในหน้า (`patch()`) ยังไม่เขียนลง store จนกดปุ่ม **"บันทึก"**. ถ้าบิลใน store เปลี่ยนจากภายนอก (เพื่อนในกลุ่มแก้ผ่าน realtime) ขณะเราไม่ได้แก้ค้าง → รับค่าใหม่มาเป็น draft
  - บิลถูกลบ/ไม่พบ id → empty state + ปุ่มกลับหน้าบิลทั้งหมด
  - แก้ชื่อบิล (inline; เตือนถ้าชื่อว่าง)
  - เลือก **วิธีหาร** — ตัวเลือกเหลือ **equal/itemized เท่านั้น** (`MODES`; โหมด `time` ปิดไว้ชั่วคราว ดู §4 — ถ้า draft เป็น `time` อยู่แล้วจะยังมีคำอธิบายว่าใช้เวลามา-กลับจากแท็บสมาชิก)
  ...
  - **เมนู** → เพิ่ม/ลบ (ต้องมีทั้งชื่อและราคาที่เป็นตัวเลขจึงเพิ่มได้); ยังไม่มีเมนู → ปุ่ม "+" มี border เน้นให้กด; ถ้าโหมด itemized โชว์ชิปเลือกผู้ร่วมต่อเมนู
  ...
  - **กล่องเตือน "บิลนี้ยังไม่เข้าสรุป"** ไล่เหตุผลจาก `billIssues` (`accessibilityRole="alert"`) — อยู่ **เหนือปุ่มบันทึก**
  - ปุ่ม **บันทึก** → เรียก `store.saveBill(draft)` (เขียนทั้งบิลรวมเมนูทีเดียว). **enable เมื่อ**: มีการแก้ไข (`dirty`) + เลือกคนออกเงินแล้ว (`paidById`) + มีเมนูอย่างน้อย 1 รายการ; ยังกดไม่ได้จะมี hint บอกเหตุผล. บันทึกสำเร็จ → เปิด **Modal (component ในแอป ไม่ใช่ dialog เบราว์เซอร์)** ให้เลือกไป "สรุปหารเงิน" (`/summary`) หรือ "บิลทั้งหมด" (`/bills`)
  - ปุ่ม **ลบบิล** (ยืนยันก่อน) → `replace('/bills')`

### 💰 สรุป (`summary.tsx`)
- **การ์ดสรุปสะอาด** (`shareCard`) — ใช้จับภาพ export: แบรนด์ 🍜 หารเมา + ชื่อกลุ่ม, ยอดรวมทุกบิล, ยอดที่แต่ละคนต้องจ่าย, และใครโอนให้ใคร (ชื่อ + ยอด)
...
- **ไม่มีหนี้ค้าง** (`nothingOwed` + มีข้อมูลให้สรุปจริง) → กล่อง 🎉 พร้อมปุ่ม **เคลียร์ทั้งหมด** (local) / **ปิดกลุ่ม**–**ออกจากกลุ่ม** (group) — ยืนยันด้วย `confirmAction` ที่ข้อความ **ต่างกัน 3 แบบ** ตาม local / host / ไม่ใช่ host (คนที่ไม่ได้สร้างกลุ่มลบกลุ่มไม่ได้ กดแล้วเป็นการออกจากกลุ่ม) แล้วเรียก `closeGroup()`
...
### 🔗 กลุ่ม + เข้าร่วม (`group.tsx`, `join/[code].tsx`) — เฉพาะเมื่อตั้ง env Supabase
...
- `group.tsx`: ยังไม่อยู่กลุ่ม → **สร้างกลุ่มใหม่** (ย้าย state local ขึ้นกลุ่ม) / **เข้าร่วมด้วยโค้ด** (ต้องยาว ≥ 4 ตัว); อยู่ในกลุ่มแล้ว → QR + invite link, ปุ่มแชร์ลิงก์ (web = คัดลอก), รายชื่อคนในกลุ่ม (ป้าย "คุณ"), ปุ่มออกจากกลุ่ม (ยืนยันด้วย `confirmAction`)
- `join/[code].tsx`: รับ code จาก deep link (`hanmao://join/<code>`) → `joinGroup(code)` → เลือก member ที่มีอยู่ (`claimMember`; ชื่อที่มีคนผูกไว้แล้วจะขึ้นหมายเหตุ แต่ยังเลือกได้เผื่อเปลี่ยนเครื่อง) หรือสร้างใหม่ (`joinAsNewMember`) → **ผูกตัวตนสำเร็จ (ทั้งสองทาง) ไปหน้าสรุปหารเงิน (`/summary`)**
  ข้อผิดพลาดทุกจุดแสดงเหตุผลจริงจาก store ผ่าน `friendlyError()` ไม่ปล่อยให้กดแล้วเงียบ (ปุ่มในหน้า error พาไปหน้ากลุ่ม `/group`)
...
| `hanmao:session:v1` | `{ groupId, myMemberId }` — อยู่กลุ่มไหนและเราเป็น member ไหน **ในกลุ่ม** เพื่อกลับเข้ากลุ่มเดิมอัตโนมัติตอนเปิดแอป |
...
> แยก me ของ local กับของกลุ่มเพราะ id ของสองฝั่งเป็นคนละชุด ถ้าเขียนข้ามกันจะได้ "ฉัน" ที่ไม่มีตัวตน (`rememberMe` เลือกที่เก็บตามโหมดปัจจุบัน)
...
- **บิล/เมนู**: `addBill(name, category)` (คืน id), `updateBill`, `saveBill(bill)`, `removeBill`, `addItem`, `updateItem`, `removeItem`, `toggleItemParticipant`
- **อื่น ๆ**: `setVenue`, `setMe(memberId | null)`, `toggleSettlement(key)`, `closeGroup()`, `reset()`
- **กลุ่ม (group)**: `createGroup(groupName)`, `joinGroup(code)`, `claimMember(memberId)`, `joinAsNewMember(name, consumes)`, `leaveGroup()`

พฤติกรรมที่ต้องรู้:

- **`isHost`** = เราเป็นคนสร้างกลุ่มนี้ไหม (เทียบ `groups.created_by` กับ uid ปัจจุบัน); local mode = `true` เสมอ
  ใช้เลือกข้อความ/ผลของปุ่มปิดกลุ่ม — **ลบกลุ่มทั้งกลุ่มได้เฉพาะ host** (RLS บังคับ ดู §6.1)
- **`closeGroup()`**: local → ล้าง state เป็นว่าง · group + host → ลบ row ของกลุ่ม (cascade ลบทุกอย่างของทุกคน) · group + ไม่ใช่ host → ลบเฉพาะแถวของตัวเองใน `group_participants` แล้วกลับ local mode
- **`saveBill(bill)`**: บันทึกทั้งบิล (ฟิลด์ + `items`) ทีเดียว — ใช้กับหน้าแก้บิลที่หน่วง draft จนกดปุ่มบันทึก
...
- **ลำดับแถวต้องนิ่ง (deterministic)**: `fetchGroupState()` สั่ง `.order()` ทั้ง 3 ตาราง (`members`/`bill_items` ตาม `created_at`, `bills` ตาม `created_at_ms`) + tiebreak `id` เสมอ แล้ว **เรียงซ้ำฝั่ง client** อีกชั้น (`sortByTimeThenId`)
  ...
  `createGroup()` ตอน migrate local → กลุ่ม ส่ง `created_at` เองด้วย `seqStamp(base, i)` ไล่ทีละ 1 ms เพราะ **`now()` ของ Postgres คงที่ทั้งทรานแซกชัน** (insert ชุดเดียวจะได้เวลาเท่ากันหมด ลำดับเดิมหาย)
...
> auth เป็น **anonymous sign-in** (`signInAnonymously`) — ต้องเปิดใน Supabase ก่อน ไม่งั้นเข้ากลุ่มไม่ได้ (login เต็มรูปเป็นงาน Phase 2)
...
- ทุกตารางเปิด **RLS** และให้สิทธิ์ผ่านการเป็นสมาชิกกลุ่มใน `group_participants`
- **`groups.created_by`** = host ของกลุ่ม; ลบกลุ่มได้เฉพาะ host, และห้ามแก้ `created_by`/`invite_code` ภายหลัง (trigger กัน)
- **เข้ากลุ่มต้องผ่าน RPC `join_group(code)`** (`security definer`) — client ไม่มีสิทธิ์ insert `group_participants` ให้ตัวเองตรง ๆ (กันแอบเพิ่มตัวเองเข้ากลุ่มคนอื่น)
...
- **Unit (`tests/unit/`)** — เทส logic ล้วน (`src/domain/**`, `src/utils/**`, `src/ui/index.ts`) โดยส่งเวลาเข้าทาง `asOf` ให้ deterministic
  รวม invariant ของโดเมนที่วัดด้วย fuzz (seed คงที่): ผลลัพธ์ไม่ขึ้นกับลำดับแถว (§4.5.1) และติ๊ก "โอนแล้ว" ต้องไม่หลุดเมื่อเวลาเดินไปเอง (§4.5) — **fuzz ครอบกลุ่ม ≤10 คน**
...
> ทำแล้ว (ย้ายออกจาก backlog): แชร์กลุ่ม real-time หลายคน (Supabase group mode) · เข้าร่วมผ่าน invite link/QR · พร้อมเพย์ให้ copy ตอนโอน · แชร์การ์ดสรุปเป็นรูป · checklist โอนแล้ว/ปิดกลุ่ม

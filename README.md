...
- **วิธีหาร**
  ...
  - `หารตามเวลา` — เฉลี่ยตามสัดส่วนเวลาที่อยู่จริง (ใช้เวลามา–กลับ) — **ปิดจาก UI ชั่วคราว** (สูตรยังอยู่ครบ บิลเก่ายังคำนวณได้)
...
- **สรุปหารเงิน** — ยอดต่อคน + คำนวณ "ใครโอนให้ใคร" ให้จำนวนโอนน้อยที่สุด (settle-up)
  ...
  เพื่อให้คู่โอนกับการติ๊กไม่เปลี่ยนเอง (เพิ่มรายการโอนได้บางกลุ่ม — ดู [ADR 0006](docs/adr/0006-stable-settle-topology.md))
- **checklist โอนแล้ว** — ติ๊กว่าโอน/รับเงินแล้ว จ่ายครบทั้งกลุ่ม → เคลียร์/ปิดกลุ่มได้
  หมายเหตุ: พอคนสุดท้ายกดว่า "กลับแล้ว" ยอดจะนิ่งเป็นตัวเลขปิดจริง → **ต้องติ๊กใหม่อีกรอบก่อนปิดกลุ่ม** (กันแอปอ้างว่าเงินโอนแล้วบนยอดที่ยังไม่นิ่ง)
...
- **กลุ่มหลายคน real-time** — สร้าง/เข้าร่วมกลุ่มผ่าน invite link หรือ QR แล้วทุกคนแก้พร้อมกันได้ (Supabase; ต้องตั้ง env)
- **เช็กพื้นที่ร้าน** — ตั้งพิกัดร้าน แล้วเช็กว่ายังอยู่ในรัศมีไหม (iOS/Android)
- **ธีมสว่าง/มืด** — ปุ่มสลับบน header ทุกหน้า (default มืด); จำค่าไว้ข้ามการเปิดแอป เป็นความชอบของเครื่อง (ไม่ผูกกับกลุ่ม)
...
### กลุ่มหลายคน real-time (group mode, ไม่บังคับ)
...
2. **รัน SQL ใน SQL Editor** (Project → SQL Editor → New query → paste → Run)
   ...
   - project **ที่ตั้งไว้ก่อนหน้านี้**: รัน `patch-NNN-*.sql` **ตามลำดับเลข** ที่ยังไม่ได้รัน
     ...
     ทุกไฟล์ idempotent รันซ้ำได้ · **`patch-004` สำคัญ**: อุดช่องโหว่ที่สมาชิกคนไหนก็ลบกลุ่มทั้งกลุ่มได้ และที่ใครรู้ id กลุ่มก็เข้ากลุ่มได้โดยไม่ต้องมีโค้ดเชิญ
   ...
   > ก่อนรันกับกลุ่มที่มีข้อมูลจริง
3. **เปิด Anonymous sign-in**: Authentication → Providers → **Anonymous** → เปิด
   (แอปใช้ `signInAnonymously()` ถ้าไม่เปิด จะสร้าง/เข้ากลุ่มไม่ได้เลย)
...
5. **ตรวจว่าใช้ได้จริง** (ไม่บังคับ แต่เร็วกว่าไปเดาในแอป):
   ...
   ```
   เช็กครบ: schema, anonymous auth, สร้างกลุ่ม, `join_group` RPC, และ RLS ไม่รั่วให้คนนอกกลุ่มเห็นข้อมูล

ถ้าไม่ตั้ง env แอปจะบังคับ local mode และซ่อนหน้ากลุ่มให้อัตโนมัติ (ไม่ error)
...
1. **เตรียม Supabase ก่อน** (ถ้าจะเปิด group mode) — ทำตามหัวข้อ "กลุ่มหลายคน real-time" ด้านบนให้ครบ:
   สร้าง project → รัน `schema.sql` → เปิด Anonymous sign-in → เก็บค่า
   `EXPO_PUBLIC_SUPABASE_URL` + `EXPO_PUBLIC_SUPABASE_ANON_KEY` ไว้
   > ข้ามขั้นนี้ได้ถ้าอยากได้แค่ local-only (เว็บใช้งานคนเดียว ไม่มีแท็บกลุ่ม)
...
5. **Deploy** → ได้ URL `https://<ชื่อ>.netlify.app` (auto-deploy ทุกครั้งที่ push)
   ยืนยัน group mode: แท็บ **"กลุ่ม"** โผล่ + สร้างกลุ่ม/เข้ากลุ่มผ่านลิงก์เชิญได้
...
app/                 หน้าจอ (expo-router, file-based routing)
  index.tsx          หน้าแรก (onboarding 3 ขั้น + ปุ่มลัด หารคนเดียว/หลายคน/เริ่มใหม่)
  ...
  group.tsx          สร้าง/จัดการกลุ่ม + QR/ลิงก์เชิญ
  join/[code].tsx    เข้าร่วมกลุ่มจาก deep link
src/
...
- กลุ่มเพื่อนประจำ + ประวัติการหาร (ปิดกลุ่มตอนนี้ = ลบถาวร ไม่เก็บย้อนหลัง)
- login เต็มรูป (link anonymous → บัญชีถาวร) + presence ใครกำลังแก้บิลไหน

> ทำเสร็จแล้ว (เคยอยู่ในลิสต์นี้): กลุ่มหลายคน real-time ผ่าน Supabase · เข้าร่วมด้วย invite link/QR · พร้อมเพย์ให้คัดลอกตอนโอน · checklist "โอนแล้ว" + ปิดกลุ่ม · แชร์การ์ดสรุปเป็นรูป

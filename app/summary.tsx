...
/**
 * ข้อความยืนยันก่อนเคลียร์/ปิดกลุ่ม — เป็นการลบถาวร ต้องบอกให้ชัดว่ากู้คืนไม่ได้
 * ในกลุ่ม: ลบกลุ่มได้เฉพาะคนที่สร้างกลุ่ม คนอื่นกดแล้วเป็นการ "ออกจากกลุ่ม" → ต้องบอกตามจริง
 */
function closeConfirmCopy(
  ...
  if (isHost) {
    return {
      title: `ปิดกลุ่ม "${groupName}" ถาวร?`,
      message:
        "สมาชิก บิล และรายการโอนของกลุ่มนี้จะถูกลบถาวรสำหรับทุกคนในกลุ่ม กู้คืนไม่ได้",
      confirmLabel: "ปิดกลุ่ม",
    };
  }
  return {
    title: `ออกจากกลุ่ม "${groupName}"?`,
    message:
      "ลบกลุ่มได้เฉพาะคนที่สร้างกลุ่ม — ยืนยันแล้วคุณจะออกจากกลุ่มนี้ และกลับไปใช้ข้อมูลในเครื่อง",
...
export default function Summary() {
  ...
  const doneCount = transfers.filter((t) =>
    state.settlements.includes(transferKey(t)),
  ).length;
  ...
  const everyoneSettled =
    hasSummary && nothingOwed(transfers, state.settlements);
  ...
  const promptPay = (id: string) =>
    state.members.find((m) => m.id === id)?.promptPay ?? null;
  ...
  const onShare = async () => {
    ...
    try {
      await shareViewAsImage(
        cardRef,
        group?.name ? `หารเมา-${group.name}` : "หารเมา-สรุป",
      );
    } catch {
  ...
  const copyPromptPay = async (pp: string, who: string) => {
    ...
    if (ok) notify("คัดลอกแล้ว", `พร้อมเพย์ของ ${who}\n${formatPromptPay(pp)}`);
      ...
      notify("คัดลอกไม่สำเร็จ", `พร้อมเพย์ของ ${who}: ${formatPromptPay(pp)}`);
  };

  const closeCopy = closeConfirmCopy(
    mode === "group",
    isHost,
    group?.name ?? "",
  );
  ...
  // ป้ายปุ่มเคลียร์/ปิดกลุ่ม — ใช้คำเดียวกับปุ่มยืนยัน ยกเว้น local mode ที่ใช้คำคุ้นเดิม
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.content}>
            ...
            {transfers.map((t) => (
              <View key={transferKey(t)} style={s.cardRow}>
                <Text style={s.rowName}>
                  <Text style={s.debtor}>{name(t.fromId)}</Text> →{" "}
                  <Text style={s.creditor}>{name(t.toId)}</Text>
                </Text>
      ...
      <Pressable
        ...
        accessibilityLabel={
          Platform.OS === "web" ? "ดาวน์โหลดรูปสรุป" : "แชร์รูปสรุป"
        }
        accessibilityHint={
          hasSummary ? undefined : "เพิ่มสมาชิกและบิลก่อนจึงจะบันทึกรูปได้"
        }
      >
        <Text style={s.shareBtnText}>{shareLabel}</Text>
      </Pressable>
      <Text style={s.hint}>
        รูปสรุปไม่มีเบอร์พร้อมเพย์ ส่งต่อในกลุ่มได้ปลอดภัย
      </Text>
      ...
      {!hasSummary && (
        <View style={s.emptyBox}>
          ...
          <Text style={s.emptyDesc}>
            เพิ่มสมาชิกและสร้างบิลก่อน แล้วยอดจะสรุปที่นี่
          </Text>
        </View>
      )}
      ...
      <Text style={s.hint}>
        คำนวณจากคนที่ออกเงินแต่ละบิล (จำนวนโอนน้อยที่สุด)
      </Text>
      {transfers.length === 0 && (
        <Text style={s.hint}>
          {hasSummary
          ...
          <View key={key} style={[s.transfer, paid && s.transferPaid]}>
            <View style={s.transferMain}>
              <Text style={[s.transferText, paid && s.paidText]}>
                <Text style={[s.debtor, paid && s.paidText]}>
                  {name(t.fromId)}
                </Text>{" "}
                →{" "}
                <Text style={[s.creditor, paid && s.paidText]}>
                  {name(t.toId)}
                </Text>
              </Text>
              <Text style={[s.transferAmt, paid && s.paidText]}>
                {baht(t.amount)}
              </Text>
            </View>
            {pp && !paid && (
              ...
              >
                <Text style={s.ppCopyText}>
                  📋 พร้อมเพย์ {formatPromptPay(pp)}
                </Text>
              </Pressable>
            )}
            {!pp && !paid && (
              <Text style={s.ppNone}>{name(t.toId)} ยังไม่ได้ใส่พร้อมเพย์</Text>
            )}
            <Pressable
              style={[s.check, paid && s.checkOn]}
              onPress={() => toggleSettlement(key)}
              {...a11y("checkbox", { checked: paid })}
              accessibilityLabel={`${name(t.fromId)} โอนให้ ${name(
                t.toId,
              )} แล้ว`}
            >
      ...
      {/* ไม่มีหนี้ค้าง (โอนครบ หรือหารลงตัวไม่ต้องโอน) → เคลียร์/ปิดกลุ่มได้ */}
      {everyoneSettled && (
        <View style={s.doneBox}>
          ...
          <Text style={s.doneDesc}>
            {mode === "group"
              ? "ปิดกลุ่มนี้เพื่อลบข้อมูลถาวรสำหรับทุกคน (กู้คืนไม่ได้)"
          ...
          <Pressable
            ...
            accessibilityLabel={
              mode === "group" ? closeCopy.title : "ลบข้อมูลทั้งหมดถาวร"
            }
          >
            ...
            {!!bd.soleBearerId && (
              <Text style={s.billNote}>
                {bill.isTreat
                  ...
                  : `${name(
                      bd.soleBearerId,
                    )} รับยอดนี้คนเดียว (ไม่มีคนเข้าเงื่อนไขบิล)`}
              </Text>
            )}
      ...
      {netRows.length === 0 && (
        <Text style={s.hint}>
          {hasSummary
            ? "ทุกคนเสมอตัว ไม่มีใครค้างใคร"
            : "ยังไม่มียอดค้าง — สร้างบิลก่อน"}
        </Text>
      )}
      {netRows.map(([id, v]) => {
      ...
      {Platform.OS !== "web" && (
        <>
          ...
          <View style={s.locRow}>
            ...
            >
              <Text style={[s.locText, { color: c.primary }]}>
                ตั้งพื้นที่ร้าน
              </Text>
            </Pressable>
            <Pressable

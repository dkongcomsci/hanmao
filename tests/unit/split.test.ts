...
describe('amountsDrift', () => {
  ...
  test('คนที่ยังไม่กลับ **ไม่ได้ร่วมบิลนั้น** (memberIds ไม่มีชื่อเขา) → เท็จ', () => {
    // เวอร์ชันเก่าใน store เช็ก s.members.some(leftAt == null) ทั้งวง = กว้างเกินไป
...
/**
 * ปิดช่องที่เกิดขึ้นได้เฉพาะตอน key **ไม่ผูกทิศทาง**: วงหลายเจ้าหนี้ (คู่คนเดิมอยู่ครบ)
 ...
 * (เคสชุดบน :748 เป็นวง 2 คนที่คู่คนมีคู่เดียว จึงไม่ครอบช่องนี้)
 */
describe('โหมด time หลายเจ้าหนี้: แก้ข้อมูลแล้วต้องเป็นโมฆะ แม้คู่คนไม่เปลี่ยน', () => {
  ...
  test('ตั้งต้น: วงนี้เป็น star หลายเส้นจริง (ไม่ใช่วง 2 คน)', () => {
  ...
  test('เปลี่ยนชื่อ/พร้อมเพย์ → ยังติด (จงใจ: ไม่กระทบยอด) แม้ในวงหลายเจ้าหนี้', () => {
    const r = check((s) => ({
      ...s,
      members: s.members.map((m) =>
        m.id === 'b' ? { ...m, name: 'สมชาย', promptPay: '0812345678' } : m,
 ...
 *
 * @param maxMembers จำนวนสมาชิกมากสุด (สุ่ม 1..maxMembers) — ใช้ 10 เพื่อวัดวงใหญ่
...
describe('invariant: ผลรวมยอดต่อคนต้องเท่ากับยอดรวมเป๊ะ (seeded fuzz 8000 เคส)', () => {
   ...
   * → กล่อง "จ่ายครบทุกคนแล้ว" + ปุ่มปิดวงหายเอง (วัดได้ 133/1946 เคส ~6.8%)
   *
   * วันนี้ต้องเป็น 0 เคส ทั้งวงเล็ก (≤5 คน) และวงใหญ่ (≤10 คน)
   * ห้ามลด ROUNDS / ห้ามลดขอบเขตเพื่อให้เขียว — แดง = star หรือ hub เพี้ยน
   */
  for (const maxMembers of [5, 10]) {
    test(`state ที่ยอดลอย: ติ๊กครบแล้วยังครบเมื่อเวลาเดินไป 1 วัน (วงไม่เกิน ${maxMembers} คน)`, () => {
      const rand = mulberry32(1122334455);
      const bad: string[] = [];
      let drifting = 0;
      for (let i = 0; i < ROUNDS; i++) {
    ...
    test(`state ที่ยอดลอย: โครงสร้างเป็นดาวรอบ hub เดียวทุกเคส (วงไม่เกิน ${maxMembers} คน)`, () => {
      const rand = mulberry32(1122334455);
      const bad: string[] = [];
      let checked = 0;
      for (let i = 0; i < ROUNDS; i++) {
     ...
     */
    test(`state ที่ยอดลอย: hub ไม่ขยับตาม asOf และไม่มีคู่คนใหม่โผล่ (วงไม่เกิน ${maxMembers} คน)`, () => {
      const rand = mulberry32(1122334455);
      const bad: string[] = [];
      let checked = 0;
      for (let i = 0; i < ROUNDS; i++) {
     ...
     */
    test(`state ที่ยอดลอย: คู่ที่หายไปต้องเป็นคนที่ net = 0 พอดี (วงไม่เกิน ${maxMembers} คน)`, () => {
      const rand = mulberry32(1122334455);
      const bad: string[] = [];
      for (let i = 0; i < ROUNDS; i++) {
        const s = randomState(rand, maxMembers);
    ...
    /**
     * ตาข่ายกันเงินหาย: ติ๊กครบตอนยอดลอย แล้วทุกคนกด "กลับแล้ว" (ปิดวง)
     ...
     */
    test(`ตาข่าย: ปิดวงแล้วติ๊กชุดเดิมเป็นโมฆะทั้งชุด ทุกเคส (วงไม่เกิน ${maxMembers} คน)`, () => {
      const rand = mulberry32(1122334455);
      const bad: string[] = [];
      let checked = 0;
      for (let i = 0; i < ROUNDS; i++) {
        ...
        if (amountsDrift(closed)) {
          bad.push(`รอบ ${i}: ปิดวงแล้วยอดยังลอย`);
          continue;
        }
        const after = settleUp(closed, ASOF);
        checked++;
        if (nothingOwed(after, ticked)) bad.push(`รอบ ${i}: ติ๊กเดิมยังนับว่าครบหลังปิดวง`);
        if (pruneSettlements(after, ticked).length !== 0) {
          bad.push(`รอบ ${i}: ยังมี key เดิมเหลือรอดหลังปิดวง`);
        }
      }
      assert.deepEqual(bad, []);
      assert.ok(checked > 100, `ตรวจได้แค่ ${checked} เคส — ชุดข้อมูลไม่ครอบ`);
    ...
    /**
     * ต้นทุนของ star: อาจใช้รายการโอนมากกว่า greedy ในวงที่มีเจ้าหนี้หลายคน
     ...
     */
    test(`ต้นทุน star: รายการโอนแพงกว่า greedy ไม่เกิน 2% และไม่เกิน +1 ต่อเคส (วงไม่เกิน ${maxMembers} คน)`, () => {
      const rand = mulberry32(1122334455);
      let starTotal = 0;
      let greedyTotal = 0;
      let worst = 0;
...
describe('regression: ยอดลอย + เครื่องหมายพลิก → คู่คนต้องไม่เปลี่ยน (ติ๊กไม่หลุด)', () => {
  ...
  test('ติ๊กครบตอน 2 ชม. → กล่อง "จ่ายครบทุกคนแล้ว" ยังอยู่ตอน 6 ชม. / 1 วัน / 30 วัน', () => {
    ...
    // ไม่มีใครแก้ข้อมูล → ต้องยังครบ (เดิมเป็น false = ปุ่มปิดวงหายเอง)
...
// ---------- ตาข่ายกันเงินหาย: ปิดวง = ต้องติ๊กใหม่บนยอดปิดจริง ----------
...
describe('ตาข่ายกันเงินหาย: ยอดหยุดลอย → ติ๊กชุดเดิมเป็นโมฆะทั้งชุด', () => {
   ...
   * ⇒ ติ๊กเดิม **ต้องเป็นโมฆะทั้งชุด** ไม่งั้นกล่อง "จ่ายครบทุกคนแล้ว" ปลดล็อกปุ่มปิดวง
  ...
  test('2 คน: ติ๊กตอนยอดลอย แล้วทุกคนกลับ → key เดิมไม่ match เลย', () => {
    ...
    assert.ok(after.length > 0, 'ปิดวงแล้วยังต้องมีรายการโอนค้าง (ไม่งั้นเทสไม่มีความหมาย)');
    assert.equal(amountsDrift(closed), false);
  ...
  test('ปิดวงแล้วติ๊กใหม่บนยอดจริง → ครบแล้วนิ่งจริง (ยอดไม่ลอยอีก)', () => {
    const closed: AppState = {
      ...driftingState(),
      members: driftingState().members.map((m) => ({ ...m, leftAt: m.leftAt ?? T1 })),
    };
 ...
 *
 * เคสนี้เกิดได้จริงในโหมดวง: src/data/remote.ts:153 ดึง members โดย **ไม่มี** .order()
